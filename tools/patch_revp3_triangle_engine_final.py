...
replacement = r'''def _conforming_watertight_solid(
    ...
    segmentized first and passed to the constrained Triangle engine. The
    resulting indexed 2D complex is extruded once; the profile warp changes
    coordinates only and therefore cannot change topology.
    """
    ...
    try:
        component = component.segmentize(max(0.25, float(max_edge) * 0.80))
    except Exception:
        pass
    component = component.buffer(0)
    ...
    print(
        f"TRI_SOLID_START area={component.area:.6f} bounds={bounds_text} "
        f"holes={len(component.interiors)} thickness={thickness:.3f}",
        flush=True,
    )

    maximum_area = max(0.30, 0.35 * float(max_edge) * float(max_edge))
    ...
    print(
        f"TRI_PLAN_DONE vertices={len(vertices_2d)} faces={len(faces_2d)}",
        flush=True,
    )
    ...
    print(
        f"TRI_EXTRUDE_DONE vertices={len(mesh.vertices)} faces={len(mesh.faces)}",
        flush=True,
    )
    ...
    print(
        f"TRI_SOLID_PASS vertices={len(mesh.vertices)} faces={len(mesh.faces)}",
        flush=True,
    )
    return mesh
'''
